...
up on information extraction. He decides to try a simple implementation of
...
# ANSWER
# When the code attempts to calculate the Inverse Document Frequency, it does
# so by invoking `Math::log` passing in the quotient of the number of
# documents # and the number of documents with the term.   This works fine if
# the quotient # of the two numbers results in a whole number, such as in 3/1.
# However, in most other cases, the quotient will misleadingly result in
# `1` since we are performing integer division.  Consequently, invoking `Math.log`
# with `1` as argument will give `0`, which in turn will guarantee that
# the term frequency - inverse document frequency is always equal to `0`.
# To solve this, we need to convert either the number of documents or the
# number of documents with the term to a float before we divide them.
